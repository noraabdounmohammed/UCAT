...
const ConceptPracticePageContent: React.FC<Omit<ConceptPracticePageProps, 'curriculumId'>> = ({ 
  ...
  const [practiceFormat, setPracticeFormat] = useState<'flashcard' | 'ukmla_sba' | 'mindmap'>('flashcard');
  ...
  const handleSaveDescription = () => {
    if (curriculum && onUpdateCurriculum && editDescription.trim()) {
      const updatedCurriculum = { ...curriculum, description: editDescription.trim() };
      onUpdateCurriculum(updatedCurriculum);
    }
            ...
            <div className="flex-1">
              ...
              {isEditingDescription ? (
                <textarea
                  ...
                  className="text-gray-600 dark:text-gray-400 max-w-2xl bg-transparent border border-blue-500 rounded p-2 focus:outline-none focus:border-blue-600 w-full resize-none"
                  rows={3}
                  autoFocus
