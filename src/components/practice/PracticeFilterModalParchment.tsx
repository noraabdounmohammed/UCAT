...
const matchesStatus = (concept: ConceptNode, values: Set<string>) => {
  ...
  return [...values].some(status =>
    ...
    (status === 'drifting' && (isDue || (!!md.attempts && !md.mastery_level)))
  );
};
...
function Chip({ selected, onClick, children, disabled = false }: { selected: boolean; onClick: () => void; children: React.ReactNode; disabled?: boolean }) {
  return (
    <button
      type="button"
      disabled={disabled}
      onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-full border px-3.5 py-2.5 text-[13px] font-semibold transition duration-150 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40"
      style={{ backgroundColor: selected ? T.espresso : 'rgba(255,253,248,.72)', color: selected ? T.cream : T.ink, borderColor: selected ? T.espresso : T.line, boxShadow: selected ? 'inset 0 1px 0 rgba(255,255,255,.08)' : '0 1px 0 rgba(31,20,12,.02)' }}
    >
      {children}
    </button>
  );
...
function ProgressChip({ selected, onClick, label, count, progress }: { selected: boolean; onClick: () => void; label: string; count: number; progress: PillProgress }) {
  ...
  return (
    <button
      ...
      style={{ background, color: T.ink, borderColor: selected ? T.espresso : T.line, boxShadow: selected ? '0 0 0 2px rgba(31,20,12,.08), inset 0 0 0 1px rgba(31,20,12,.08)' : '0 1px 0 rgba(31,20,12,.02)' }}
    >
...
export const PracticeFilterModalParchment: React.FC<Props> = ({ isOpen, onClose, onApplyFilters, initialFilters = null, defaultSize = 10 }) => {
  ...
  // The catalogue is populated asynchronously for a fresh/incognito learner. Re-read
  // its metadata whenever the modal opens or the concept store changes, rather than
  // freezing the empty localStorage snapshot captured on the first render.
  useEffect(() => {
    if (!isOpen) return;
    const storedCategories = getStorage<FilterCategory[]>(cid, 'filter_categories', []);
    const storedAssignments = getStorage<Record<string, string>>(cid, 'filter_assignments', {});
    setCategories(filterCategories.length ? filterCategories : storedCategories);
    setAssignments(storedAssignments);
    if (concepts.length > 0) setRetryingCatalog(false);
  }, [cid, isOpen, concepts.length, filterCategories]);
  ...
  const retryCatalog = async () => {
    ...
    try {
      await loadConcepts();
      setCategories(filterCategories.length ? filterCategories : getStorage<FilterCategory[]>(cid, 'filter_categories', []));
      setAssignments(getStorage<Record<string, string>>(cid, 'filter_assignments', {}));
    } finally {
  ...
  const specialtyCategoryId = (categoryFor('specialty') || categoryFor('system'))?.id;
  const conditionCategoryId = categoryFor('condition')?.id;
  const presentationCategoryId = categoryFor('presentation')?.id;
  const facetCategoryId = (categoryFor('other') || categoryFor('skill') || categoryFor('facet') || categoryFor('topic'))?.id;
  const specialtyIds = useMemo(() => specialtyCategoryId ? tagsByCategory[specialtyCategoryId] || [] : [], [specialtyCategoryId, tagsByCategory]);
  const conditionIds = useMemo(() => conditionCategoryId ? tagsByCategory[conditionCategoryId] || [] : [], [conditionCategoryId, tagsByCategory]);
  const presentationIds = useMemo(() => presentationCategoryId ? tagsByCategory[presentationCategoryId] || [] : [], [presentationCategoryId, tagsByCategory]);
  const facetIds = useMemo(() => facetCategoryId ? tagsByCategory[facetCategoryId] || [] : [], [facetCategoryId, tagsByCategory]);
  const visibleConditionIds = useMemo(() => essentialsOnly ? conditionIds.filter(isEssentialTag) : conditionIds, [conditionIds, essentialsOnly]);
  const visiblePresentationIds = useMemo(() => essentialsOnly ? presentationIds.filter(isEssentialTag) : presentationIds, [essentialsOnly, presentationIds]);
  ...
  const catalogLoading = concepts.length === 0 && (isLoading || !retryingCatalog);

  const handleStart = () => {
    const selectedConcepts = sStatus.has('cold')
      ? pickColdBreadthFirst(filteredPool, size, assignments, conditionCategoryId, presentationCategoryId)
      : filteredPool;
    setPracticeSelection(selectedConcepts.map(concept => concept.concept_id));
    onApplyFilters?.({
      size,
      statuses: [...sStatus],
      areas: [...sAreas],
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center md:items-center md:p-5" style={{ backgroundColor: 'rgba(31,20,12,0.24)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)' }} onClick={onClose}>
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="practice-filter-title" className="flex h-[100dvh] w-full flex-col overflow-hidden rounded-none shadow-[0_-18px_60px_rgba(31,20,12,0.16)] md:h-auto md:max-h-[90vh] md:max-w-[470px] md:rounded-[30px] md:border" style={{ backgroundColor: T.cream, borderColor: 'rgba(217,204,182,.8)', fontFamily: "'Inter', sans-serif", animation: 'studyedit-sheet-in 260ms cubic-bezier(.2,.8,.2,1) both' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-5">
          ...
          {concepts.length === 0 ? (
            <section className="border-t py-10 text-center" style={{ borderColor: T.lineSoft }} aria-live="polite">
              <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-transparent border-t-current" style={{ color: T.inkMuted }} />
              <div className="mt-4 text-[16px] font-semibold" style={{ color: T.ink }}>{retryingCatalog ? 'Reloading your curriculum…' : 'Loading your UKMLA curriculum…'}</div>
              <div className="mx-auto mt-2 max-w-[280px] text-[12px] leading-5" style={{ color: T.inkMuted }}>Your filter options will appear as soon as the concept catalogue is ready.</div>
              {!isLoading && <button type="button" onClick={() => void retryCatalog()} disabled={retryingCatalog} className="mt-5 rounded-full border px-5 py-2.5 text-[12px] font-semibold disabled:opacity-50" style={{ borderColor: T.line, color: T.ink, backgroundColor: 'rgba(255,253,248,.72)' }}>{retryingCatalog ? 'Retrying…' : 'Retry loading'}</button>}
            </section>
          ) : (
            <>
