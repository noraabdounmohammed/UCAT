    ...
    (set, get) => ({
      ...
      loadConcepts: async () => {
        ...
        try {
          ...
          const filterOptions: ConceptFilterOptions = {
            // UKMLA-specific options
            systems: data.dimensions?.systems || [],
            ...
            competencies: data.dimensions?.competencies || [],
            // Generic options
            domains: data.dimensions?.domains || [],
            subjects: data.dimensions?.subjects || [],
            topics: data.dimensions?.topics || [],
            subtopics: data.dimensions?.subtopics || [],
      ...
      addConcept: (newConcept: Partial<ConceptNode>) => {
        ...
        // Update state and recalculate filtered concepts
        const filteredConcepts = filterConcepts(updatedConcepts, currentState.filterState);
        const stats = calculateStats(filteredConcepts);
