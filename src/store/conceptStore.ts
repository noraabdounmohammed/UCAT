...
const extractFilterOptions = (concepts: ConceptNode[]): ConceptFilterOptions => {
  return {
    ...
    // Generic options
    domains: Array.from(new Set(concepts.map(c => c.dimensions?.domain).filter((d): d is string => Boolean(d)))),
    subjects: Array.from(new Set(concepts.map(c => c.dimensions?.subject).filter((s): s is string => Boolean(s)))),
    topics: Array.from(new Set(concepts.map(c => c.dimensions?.topic).filter((t): t is string => Boolean(t)))),
    subtopics: Array.from(new Set(concepts.map(c => c.dimensions?.subtopic).filter((st): st is string => Boolean(st)))),
    ...
    (set, get) => ({
      ...
      startPractice: async (practiceConfig?: PracticeConfig) => {
        ...
        if (practiceConfig) {
          ...
          // Filter by question formats (skip for flashcards since any concept can be a flashcard)
