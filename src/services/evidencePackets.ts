...
export const LAUNCH_EVIDENCE_PACKETS: Record<string, EvidencePacket> = {
  ...
  'ukmla-176': packet(
    ...
    ['sex', 'CHA2DS2-VASc score', 'bleeding risk', 'explicit statement that a DOAC is suitable or unsuitable when class choice is tested', 'relevant contraindications'],
    ['whether anticoagulation is indicated', 'broad anticoagulant class only: DOAC versus VKA'],
    ['Never name or dose a specific DOAC in this item.', 'Do not substitute antiplatelet therapy for indicated anticoagulation.', 'Do not make warfarin preferred when the stem explicitly states a DOAC is suitable.'],
    ['no anticoagulation despite indication', 'aspirin instead of anticoagulation', 'vitamin K antagonist despite explicit DOAC suitability'],
    'NICE NG196',
    'critical',
  ),
  'ukmla-184': packet(
